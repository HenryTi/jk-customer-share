...
export async function showIDUnitCustomer(jkCustomer: JkCustomer.UqExt, unit:number) {
	await showIDCustomer(jkCustomer, jkCustomer.UnitCustomer, unit);
}

async function showIDCustomer(jkCustomer: JkCustomer.UqExt, IX: IX, ix?:number) {
	let IDUI: IDUI = {
	...
	let mId = new MidIXID<Customer>(jkCustomer, IDUI, IX, ix);
	mId.listHeader = '客户列表';
	mId.itemHeader = '客户';
